...
def gaussion_prob(x, mu, var):  # 
    prob = 1 * np.exp(- np.power((x - mu), 2) / (2 * var)) / np.sqrt(2 * np.pi * var)
...
if __name__ == "__main__":
    ...
    # img_train_project = PCA(img_train_flatten, k=50)
    # img_test_project = PCA(img_test_flatten, k=50)

    bayes = BayesClassifier(independent=True)
